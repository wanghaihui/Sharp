package com.conquer.sharp.emoji;
...
public class EmojiParser {
    ...
    private  List<Integer> gifIconIdsList;

    private int gifIconNum = 6;
    ...
    private EmojiParser() {
        ...
        gifIconIdsList = new ArrayList<Integer>();
        for (int i = Emojis.sIconIds.length - gifIconNum; i < Emojis.sIconIds.length; i++) {
            gifIconIdsList.add(Emojis.sIconIds[i]);
        }
    }

    public boolean isGifDrawable(int position){
        if(position >=  Emojis.sIconIds.length - gifIconNum && position< Emojis.sIconIds.length){
            return true;
        }
        return false;
    }
    static class Emojis {
        private static final int[] sIconIds = {
                ...
                // GIF drawable
                R.drawable.emoji_gif_lianmai, R.drawable.emoji_gif_shangmai,
                R.drawable.emoji_gif_hai, R.drawable.emoji_gif_saqian, R.drawable.emoji_gif_baobao,
                R.drawable.emoji_gif_sex
        };

        private static final int[] giftIconIds = {
                R.drawable.emoji_gif_lianmai, R.drawable.emoji_gif_shangmai,
                R.drawable.emoji_gif_hai, R.drawable.emoji_gif_saqian, R.drawable.emoji_gif_baobao,
                R.drawable.emoji_gif_sex
        };
    ...
    public static Integer[] DEFAULT_EMOJI_RES_IDS;
    ...
    private HashMap<String, Integer> buildSmileyToRes() {
        ...
        if (mEmojiTexts.length != mSoftbankEmojiTexts.length) {
            ...
            throw new IllegalStateException("Smiley resource ID/text mismatch - "
                    + mEmojiTexts.length + " != " + mSoftbankEmojiTexts.length);
        }

        // Initialize the resource ids
        DEFAULT_EMOJI_RES_IDS = new Integer[mSoftbankEmojiTexts.length];
     ...
     *         emojis
     */
    public CharSequence addEmojiSpans(CharSequence text, Context mContext) {
    ...
    public CharSequence addEmojiGifSpans(CharSequence text, Context mContext, TextView tv) {
        return addEmojiGifSpans(text, mContext, tv, true);
    }
    ...
    public CharSequence addEmojiGifSpans(CharSequence text, Context mContext, TextView tv,
        ...
        while (matcher.find()) {
            int resId = mSmileyToRes.get(matcher.group());
            try {
                VerticalImageSpan span = null;
                if (isGif(resId)) {
                    if (isShowGif) {
                        /*GifDrawable gifDrawable = new GifDrawable(mContext.getResources(),getGifDrawble(gifIconIdsList.indexOf(resId)));
                        gifDrawable.setBounds(0, 0, (int)(gifDrawable.getIntrinsicWidth() *1.5),
                                (int)(gifDrawable.getIntrinsicHeight() * 1.5));
                        gifDrawable.setCallback(tv);
                        span = new VerticalImageSpan(gifDrawable);*/
                    } else {
                        return builder;
                    }
                } else {
                    span = new VerticalImageSpan(mContext, resId);
                }
    ...
    private int getGifDrawble(int i){
        if(i <  Emojis.giftIconIds.length && i >=0){
            return Emojis.giftIconIds[i];
        }
        return Emojis.giftIconIds[0];
    }
    public static CharSequence addDrawbleSpans(CharSequence text, Context mContext, int resId) {
        SpannableStringBuilder builder = new SpannableStringBuilder(" "+text);
        // Scan for both Softbank private Unicode emoji code points (every
        // Android SMS app + iOS < 5) and Unicode 6.1 emoji BMP and non-BMP
        // codes
        try {
            VerticalImageSpan span = new VerticalImageSpan(mContext, resId);
    ...
    private boolean isGif(int resId) {
        return gifIconIdsList.contains(resId);
    }
}
