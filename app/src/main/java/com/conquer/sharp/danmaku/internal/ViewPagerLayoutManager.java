...
public abstract class ViewPagerLayoutManager extends LinearLayoutManager {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            // 重新布局，经历onMeasure, onLayout, onDraw
            requestLayout();
        }
    }
    ...
    public void setMaxVisibleItemCount(int maxVisibleItemCount) {
        assertNotInLayoutOrScroll(null);
        if (mMaxVisibleItemCount == maxVisibleItemCount) {
            return;
        }
        mMaxVisibleItemCount = maxVisibleItemCount;
        removeAllViews();
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        // make sure properties are correct while measure more than once(不止一次)
    ...
    public int getDistanceToBottom() {
        return mDistanceToBottom == INVALID_SIZE ? (mOrientationHelper.getTotalSpaceInOther() - mDecoratedMeasurementInOther) / 2 : mDistanceToBottom;
    }

    public void setDistanceToBottom(int distanceToBottom) {
        assertNotInLayoutOrScroll(null);
        if (mDistanceToBottom == distanceToBottom) {
            return;
        }
