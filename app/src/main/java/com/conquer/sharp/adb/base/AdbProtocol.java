...
public class AdbProtocol {
    ...
    /** This authentication(身份认证) type represents a SHA1 hash to sign */
    ...
    private static int getPayloadChecksum(byte[] payload) {
        int checksum = 0;
        for (byte b : payload) {
            /* We have to manually "unsign" these bytes because Java sucks(糟糕) */
            if (b >= 0) {
                checksum += b;
            } else {
                checksum += (b + 256);
    ...
    final static class AdbMessage {
        ...
        /** command identifier constant (CMD_CNXN, ...) */
        public int command;
        ...
        /** second argument                           */
        public int arg1;
        /** The payload(有效荷载) length field of the message */
        /** length of payload (0 is allowed)          */
        public int payloadLength;
        /** The checksum(校验和) field of the message */
         ...
         * This message is NOT validated(未验证的).
